...
export default function Marketplace({ courses }) {
  ...
  const purchaseCourse = async (order, course) => {
    const hexCourseId = Web3Utils.utf8ToHex(course.id);
    const orderHash = Web3Utils.soliditySha3(
      { type: "bytes16", value: hexCourseId },
      { type: "address", value: account.data }
    );

    const value = Web3Utils.toWei(String(order.price));
  ...
  const _purchaseCourse = async ({ hexCourseId, proof, value }, course) => {
    try {
      const result = await _web3Api.contract.methods
