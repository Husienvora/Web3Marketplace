...
export default function ManagedCourses() {
  ...
  const verifyCourse = (email, { hash, proof }) => {
    ...
    const emailHash = Web3Utils.sha3(email);
    const proofToCheck = Web3Utils.soliditySha3(
  ...
  const changeCourseState = async (courseHash, method) => {
    try {
      const result = await _web3Api.contract.methods[method](courseHash).send({
        from: account.data,
      });
      return result;
    } catch (e) {
  ...
  const searchCourse = async (hash) => {
    ...
    if (hash && hash.length === 66 && re.test(hash)) {
      const course = await _web3Api.contract.methods
